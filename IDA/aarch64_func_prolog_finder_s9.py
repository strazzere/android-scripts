...
def is_prologue_insn(ea):
    ...
    return ida_ua.insn_t().itype is idaapi.ARM_eor #in [idaapi.ARM_stp, idaapi.ARM_mov, idaapi.ARM_sub]

def callback(ea):
    flags = ida_bytes.get_full_flags(ea)
    if ida_bytes.is_unknown(flags):
        while ea != ida_idaapi.BADADDR:
            if is_prologue_insn(ea - 8):
                ea -= 8
            else:
                print("[*] New function discovered at %#lx" % (ea - 8))
                idaapi.add_func(ea - 8, ida_idaapi.BADADDR)
                break
